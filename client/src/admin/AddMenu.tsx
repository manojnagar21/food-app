                    ...
                    <DialogContent>
                        ...
                        <form onSubmit={submitHandler} action="" className="space-y-4">
                            <div>
                                ...
                                />
                                { error && <span className="tyext-xs font-medium text-red-600">{error.name}</span> }
                            </div>
                            <div>
                                ...
                                />
                                { error && <span className="tyext-xs font-medium text-red-600">{error.description}</span> }
                            </div>
                            <div>
                                ...
                                />
                                { error && <span className="tyext-xs font-medium text-red-600">{error.price}</span> }
                            </div>
                            <div>
                                ...
                                />
                                { error && <span className="tyext-xs font-medium text-red-600">{error.image?.name }</span> }
                            </div>
                ...
                restaurant?.menus.map((menu: any, idx: number) => (
                    <div key={idx} className="mt-6 space-y-4">
                        <div className="flex flex-col md:flex-row md:items-center md:space-x-4 md:p-4 p-2 shadow-md rounded-lg border">
                            ...
                            <Button onClick={() => { setSelectedMenu(menu); setEditOpen(true); }} size={'sm'} className="bg-orange hover:bg-hoverOrange mt-2">Edit</Button>
                        </div>
                    </div>
                ))
            }
