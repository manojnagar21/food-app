...
// dotenv.config();
const API_END_POINT = 'https://food-mrjyyvgpc-mritunjay-natwarlal-nagars-projects.vercel.app/api/v1/restaurant';
...
export const useRestaurantStore = create<RestaurantState>()(persist((set, get) => ({
    ...
    // update restaurant api implementation
    ...
    // search restaurant api implementation
    ...
    updateRestaurantOrder: async (orderId: string, status: string) => {
        try {
            ...
            // set({ restaurantOrder: response.data });
        } catch (error: any) {
            toast.error(error.response.data.message);
        }
    },
