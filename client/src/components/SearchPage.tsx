...
const SearchPage = () => {
    ...
    const { loading, searchRestaurant, searchedRestaurant, setAppliedFilter, appliedFilter } = useRestaurantStore();
    useEffect(() => {
        searchRestaurant(params.text!, searchQuery, appliedFilter);
        // console.log(searchRestaurant(params.text!, searchQuery, appliedFilter));
                    ...
                    <div>
                        <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-2 my-3">
                            <h1 className="font-medium text-lg">
                                ({ searchedRestaurant?.data.length }) Search results found
                            </h1>
                                    ...
                                    appliedFilter.map((selectedFilter: string, idx: number) => (
                                        <div key={idx} className="relative inline-flex items-center max-w-full">
                                            <Badge 
                                                ...
                                                variant="outline">{selectedFilter}
                                            </Badge>
                                            <X
                                        ...
                                        searchedRestaurant?.data.map((restaurant: Restaurant) => (
                                            <Card key={restaurant._id} className="bg-white dark:bg-gray-800 shadow-xl rounded-xl overflow-hidden hover:shadow-2xl transition-shadow duration-300">
                                                ...
                                                <CardContent className="p-4">
                                                    ...
                                                    <div className="mt-2 gap-1 flex items-center text-gray-600 dark:text-gray-100">
                                                        <MapPin size={16} />
                                                        <p className="text-sm">
                                                            City:{" "}
                                                    ...
                                                    <div className="mt-2 gap-2 flex items-center text-gray-600 dark:text-gray-100">
                                                        <Globe size={16} />
                                                        <p className="text-sm">
                                                            Country:{" "}
                                                    ...
                                                    <div className="flex gap-2 mt-4 flex-wrap">
                                                        {
                                                            restaurant.cuisines.map((cuisine: string, idx: number) => (
                                                                <Badge 
                                                                    key={idx} 
                                                                    className="font-medium px-2 py-1 rounded-full shadow-sm">{cuisine}
                                                                </Badge>
                                                            ))
                                                        }
                                                    </div>
